...
```vue-html
<div id="app">
  <button onclick="count++; this.innerText = 'Count is: ' + count;">
    Count is: <span id="count-value">0</span>
  </button>
  <script>
    let count = 0;
    document.querySelector('#app button').onclick = function() {
      count++;
      this.querySelector('#count-value').innerText = count;
    };
  </script>
</div>
```
...
<Card title="Read the Guide" class="next-steps-link">
