...
description: Vue's template syntax allows you to declaratively bind the rendered DOM to the underlying component instance's data.
---

import { Card } from "@astrojs/starlight/components";
import VueSchoolLink from "/src/components/VueSchoolLink.vue";
import RawHtmlDemo from "./RawHtmlDemo/RawHtmlDemo.vue";

<VueSchoolLink
  href="https://scrimba.com/links/vue-template-syntax"
  title="Watch an interactive video lesson on Scrimba"
/>

Vue uses an HTML-based template syntax that allows you to declaratively bind the
rendered DOM to the underlying component instance's data. All Vue templates are syntactically
...
a **directive**. Directives are prefixed with `v-` to indicate that they are special
attributes provided by Vue, and as you may have guessed, they apply special reactive
behavior to the rendered DOM. Here, we're basically saying "keep this element's inner
HTML up-to-date with the `rawHtml` property on the current active instance."
